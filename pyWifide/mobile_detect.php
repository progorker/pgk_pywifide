 ...
 * @return bool
 *
 * Source: https://github.com/serbanghita/Mobile-Detect/blob/4.x/src/MobileDetect.php
 * License: MIT
 */
function check_http_headers_for_mobile(): bool {
  $knownUserAgentHttpHeaders = [
  ...
  $knownCloudFrontHeaders = [
        'HTTP_CLOUDFRONT_IS_MOBILE_VIEWER',
        'HTTP_CLOUDFRONT_IS_TABLET_VIEWER',
        'HTTP_CLOUDFRONT_IS_DESKTOP_VIEWER'
    ];
    
  $header_keys = array_merge(
    array_values($knownUserAgentHttpHeaders),
    array_keys($knownMobilePositiveHeaders),
    array_values($knownCloudFrontHeaders)
  );
        
  $httpHeaders = [];
  foreach ($header_keys as $key) {
    if (isset($_SERVER[$key])) {
      $httpHeaders[$key] = $_SERVER[$key];
    }
  }
        
  $userAgent = "";
  foreach ($knownUserAgentHttpHeaders as $altHeader) {
    if (!empty($httpHeaders[$altHeader])) {
      $userAgent .= $httpHeaders[$altHeader] . " ";
    }
  }

  foreach ($knownMobilePositiveHeaders as $mobileHeader => $matchType) {
    if (isset($httpHeaders[$mobileHeader])) {
      if (isset($matchType['matches']) && is_array($matchType['matches'])) {
        foreach ($matchType['matches'] as $_match) {
          if (str_contains($httpHeaders[$mobileHeader], $_match)) {
            return true;
          }
        }
        return match_user_agent_with_first_found_matching_rule( $userAgent );
      } else {
        return true;
      }
    }
 ...
 * License: MIT
 */
function g_match(string $regex, string $userAgent): bool {
 ...
 * License: MIT
 */
function match_user_agent_with_first_found_matching_rule( $userAgent ): bool {
  $browsers = [
        //'Vivaldi'         => 'Vivaldi',
  ...
  foreach ($rules as $_regex) {
    ...
    if (is_array($_regex)) {
      foreach ($_regex as $regexString) {
        if (g_match($regexString, $userAgent)) {
          return true;
        }
      }
    } else {
      // assume regex is "string"
      if (g_match($_regex, $userAgent)) {
        return true;
      }
    }
  }
